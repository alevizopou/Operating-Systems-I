...
#	pare apostasi
	...
	for (( c=1; c<=20; c++ )) 
		do
			...
			r=$(./client ${pizzes[@]} $apostasi | grep Server)  # emfanise mono to apotelesma tis paraggelias
			echo "client $client order $c : $r"
		done
}
...
for (( p=1; p<=$clients ; p++ ))
	do
		run_client $p & # trexoume kathe run client sto background => trexoun parallila
	done
